...
每次执行全量自动化测试时，在本地生成带测试时间的独立记录，保存完整原始输出、结构化元数据和简要分析，方便后续 Agent 与人工追溯。测试记录不提交 Git。根据首次记录修复稳定复现的产品缺陷，再生成修复后记录并分析前后差异。
...
- 允许修改业务代码，范围仅限测试记录中稳定复现的缺陷及其必要回归保障。
...
实现记录功能后先执行一次 `npm run test:all`，生成修复前记录。根据该记录修复当前四项稳定缺陷：

1. `src/crypto.js`：Base58 全零输入不得在前导零编码之外额外追加 `1`。
2. `src/store.js`：将状态读取、变更和持久化作为一个串行事务，防止并发更新丢失。
3. `src/server.js`：读取 JSON 请求体前校验 `Content-Type`，非 `application/json` 返回 HTTP 415 和稳定错误码。
4. `src/server.js`：静态资源、API 成功响应和错误响应统一设置 CSP、`X-Content-Type-Options: nosniff` 与 `Referrer-Policy`。

修复过程遵循现有失败测试先行：

- 每项修复前单独运行对应测试，确认仍按预期失败。
- 实施最小业务代码修改。
- 单独运行对应测试，确认转为通过。
- 运行相关测试层，确认无回归。
- 最后执行 `npm run test:all` 生成修复后记录。

修复后 `result.md` 应能与最近一次修复前记录比较，简要列出失败数变化、通过率变化、已解决缺陷和新增失败。若无法自动定位上一份记录，报告仍输出本次分析，并明确未生成趋势对比。

`docs/测试缺陷报告.md` 保留原复现信息，将通过回归的缺陷标记为已修复，记录修复时间、修复提交和验证命令。
...
现有四条失败测试作为业务修复的回归测试。修复完成后，Node 106 条和 Chromium UI 6 条测试都必须通过，`npm run test:all` 返回退出码 0。
...
- 修复前记录保留四项失败，修复后记录显示四项缺陷已解决且无新增失败。
- `docs/测试缺陷报告.md` 中四项缺陷均包含修复状态和回归证据。
