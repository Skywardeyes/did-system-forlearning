...
> 状态：个人钱包 MVP 已实施并完成自动化验证；生产级的跨设备恢复、正式 DID Registry、外部 HSM 与 Challenge 防重放台账仍在后续范围。
...
- V6 迁移把 DID 的唯一约束改为“租户 + DID”，允许多个机构登记同一公开 Holder DID。
...
4. 回归：Node 测试、Vue 构建、数据库 V6 健康检查和 V2 冒烟测试通过。
...
| Challenge 一次性消费 | 页面提示，未持久化 | 增加 verifier nonce 台账与原子消费 |
...
> 我们没有把“服务端加密保存 Holder 私钥”当作自主管理身份。当前 MVP 已把 Holder 密钥生成、保存和出示签名移到独立个人钱包，平台只登记公钥 DID Document；机构 Issuer 密钥才由 KMS 管理。钱包已能完成本地最小披露和 Holder 签名绑定。正式 Registry、设备恢复、标准 Holder Binding 和挑战防重放属于下一阶段，不能夸大为已经完成。
