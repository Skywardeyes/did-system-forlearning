...
大家好，我们的项目是“信证台：DID/VC 本地演示系统”。系统以培训结业证书为业务场景，完成 Issuer 和 Holder 身份创建、VC 签发、七项验证、选择性披露、篡改检测、生命周期管理和审计追踪。
...
| FR-09 | 选择性披露 | 支持教学版“随机盐 + SHA-256 摘要清单”与 RFC 9901 核心 SD-JWT；均只提交选中声明并保存带格式标识的披露验证台账 |
...
| 领域服务 | `src/vc-service.js` | DID/VC 规则、生命周期、完整验证和选择性披露验证 |
...
项目没有关系型数据库，下面采用“实体卡片 + 关系箭头”的 ER 表达风格，对 `store.json` 和 `logs.json` 中领域对象进行逻辑建模；它不表示实际建立了 SQL 表，中文字段释义用于现场讲解。
...
flowchart LR
    ...
    KEY["<b>KEY_HISTORY｜历史密钥</b><br/>didId：所属 DID（FK）<br/>version：密钥版本<br/>publicJwk：历史公钥<br/>retiredAt：停用时间"]
    ...
    VERIFY["<b>VERIFICATION_LOG｜验证记录</b><br/>credentialId：被验证凭证（FK）<br/>valid：总体结果<br/>failedChecks：失败检查项<br/>checkedAt：验证时间"]
    LOG["<b>STRUCTURED_LOG｜结构化日志</b><br/>correlationId：请求关联号<br/>type / level：类型与级别<br/>module / action：模块与动作<br/>success：执行结果"]
    DID -->|"1 对多：保留轮换历史"| KEY
    DID -->|"1 对多：作为签发方"| VC
    DID -->|"1 对多：作为持有方"| VC
...
系统为姓名、课程、完成日期和完成状态分别生成 16 字节随机盐，再计算 `SHA-256(stableStringify({ path, salt, value }))`。Issuer 使用 Ed25519 对包含全部声明摘要、凭证 ID、有效期和 Issuer 的摘要清单签名。Holder 生成披露证明时只提交选中字段的路径、值和盐；未选择字段只在清单中保留摘要，原始值和盐不会进入证明或普通 API。

教学版验证方重新计算已公开字段摘要，并检查 Issuer、当前或历史密钥、摘要清单签名、有效期和 VC 状态。新增的 RFC 9901 核心 SD-JWT 则由 Issuer 以 EdDSA 签发 JWT，在 `_sd` 中保存 Disclosure 摘要；Holder 只提交 `IssuerJWT~选中 Disclosure~`，验证方验 JWS、重算 Disclosure 摘要并检查其属于 `_sd`。两条路线的成功或失败都会写入独立披露证明验证台账，记录凭证 ID、格式、公开字段、失败项和验证时间。教学版协议不是 BBS+ 或零知识证明；SD-JWT 当前未实现 Holder key binding、钱包本地生成或完整 SD-JWT VC 应用配置。
...
sequenceDiagram
    ...
    VC-->>UI: valid（总体结论）+ checks（分项结果）+ failedChecks（失败项）
```
...
| 测试层 | 主要覆盖 |
|---|---|
| 单元测试 | Stable JSON、Base58、Ed25519、加盐摘要、DID Method、查询分页、脱敏和 UI Helper |
| 集成测试 | JSON Store、日志、DID/VC Service、选择性披露、状态机和历史密钥 |
| API 测试 | DID/VC CRUD、披露生成验证、生命周期、错误 JSON、分页响应和隐私边界 |
| 功能测试 | 四种 Method 组合、篡改、轮换、停用、暂停恢复、替换撤销和部分字段披露 |
| 安全测试 | 私钥与披露材料泄露、日志敏感字段、XSS、原型污染、目录穿越、安全响应头 |
| UI 测试 | 44 条 Chromium 用例覆盖六个入口、身份、签发、完整验证、选择性披露与台账、生命周期、日志和桌面/移动布局 |
...
flowchart LR
    subgraph EXEC["① 执行测试"]
        Run["统一测试入口<br/>npm run test:evidence"] --> Node["Node 自动化测试<br/>单元、集成、API 与安全"]
        Run --> UI["Chromium 界面测试<br/>真实用户操作流程"]
    end
    subgraph ARCHIVE["② 永久归档"]
        Record["创建带时间戳的测试批次"] --> Raw["原始输出<br/>node.log / ui.log"]
        Record --> Report["结果与环境<br/>result.md / metadata.json"]
        Record --> Browser["用例与浏览器报告<br/>test-cases.json / Playwright 报告"]
    end
    subgraph TRUST["③ 完整性校验"]
        Manifest["证据清单与 SHA-256 校验值<br/>识别缺失、新增或修改"] --> Verify["重新校验证据<br/>npm run test:evidence:verify"]
    end
    subgraph TRACE["④ 历史追溯"]
        List["查询历次测试批次<br/>npm run test:evidence:list"]
    end
    Node --> Record
    UI --> Record
    Raw --> Manifest
...
- 最新代码已实际复核 `npm run test:node`：130 项，130 项通过，本轮约 2.4 秒；
- 当前代码已实际复核 44 项 Chromium UI 测试全部通过，其中包括选择字段、生成与验证披露证明、隐藏姓名、篡改失败、披露验证台账和移动布局；
...
前面从软件工程角度介绍了系统如何从需求落实到设计、代码和测试。接下来由 B 使用真实系统，验证身份查看、凭证签发、七项验证、篡改检测、完整 VC 生命周期和 RFC 9901 核心 SD-JWT 选择性披露，并与教学版摘要证明进行对比。
...
| 15:20–16:00 | 撤销新 VC | 撤销是不可逆终态 | 签名可通过但 credentialStatus 失败 |
| 16:00–16:40 | 选择性披露 | 选择 SD-JWT，只公开课程和完成日期；Issuer JWT 的 `_sd` 约束 Disclosure 摘要 | 验证通过并写入带格式标识的披露台账；篡改课程后摘要失败 |
| 16:40–17:00 | 查看日志 | 业务操作、correlationId 和脱敏详情 | 可找到刚才的操作记录 |

### 演示备用原则

- 原始 VC 在篡改前单独保存，后续暂停、恢复验证使用原始 JSON；
- 替换后的新 VC 立即打开或复制，供撤销后验证；
- 超时则减少 JSON 字段解释，把撤销压缩为口头说明；保留正常验证、篡改、暂停恢复和选择性披露；
...
- [x] 最新 Node 测试已复核为 132/132（含 RFC 9901 核心 SD-JWT）
- [ ] Chromium UI 44/44 为新增 SD-JWT 页面前基线；答辩前须复跑并归档最终证据
