...
| FR-09 | 选择性披露 | 使用随机盐与 SHA-256 生成声明摘要，由 Issuer 签名摘要清单；只提交选中声明并保存披露验证台账 |
...
验证方重新计算已公开字段摘要，并检查 Issuer、当前或历史密钥、摘要清单签名、有效期和 VC 状态。每次成功或失败都会写入独立披露证明验证台账，记录凭证 ID、公开字段、失败项和验证时间。该协议是教学实现，不是 BBS+、零知识证明或正式 SD-JWT VC。
...
前面从软件工程角度介绍了系统如何从需求落实到设计、代码和测试。接下来由 B 使用真实系统，验证身份查看、凭证签发、七项验证、篡改检测、完整 VC 生命周期和教学版选择性披露。
...
| 16:00–16:40 | 选择性披露 | 只公开课程和完成日期；Issuer 签名摘要清单 | 验证通过并写入披露台账；篡改课程后摘要失败 |
