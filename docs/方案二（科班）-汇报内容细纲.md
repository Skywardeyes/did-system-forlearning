...
| FR-06 | 查询展示 | DID、VC、验证记录和日志支持搜索、排序、分页 |
...
| UI 测试 | 导航、身份、签发、验证、生命周期、日志和布局 |

#### 6.3 当前验证结果

- 已实际执行 `npm run test:node`：120 项，120 项通过；
- Node 测试运行约 2.8 秒；
- 当前环境未安装 `@playwright/test`，因此本轮未实际执行 UI 测试；
- 默认 `npm test` 的文件发现范围会误执行 `test/helpers/playwright-server.js` 并使进程不退出，当前使用 `npm run test:node`。
...
| 系统测试 | 补充测试场景、分析失败输出 | 判断测试是否有效、复现并验收 | `test/`、缺陷报告、测试记录 |
...
- [ ] 正式汇报前安装 Playwright、执行 UI 测试并更新结果
- [ ] 处理或说明默认 `npm test` 的已知入口问题
