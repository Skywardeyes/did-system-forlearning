...
test('schema compatibility accepts V1 through V10 during staged repository migration', async () => {
  ...
  await assert.rejects(() => assertSupportedSchema(poolAt(9), { requiredVersion: 10 }), { code: 'SCHEMA_VERSION_UNSUPPORTED' });
  await assert.rejects(() => assertSupportedSchema(poolAt(11)), { code: 'SCHEMA_VERSION_UNSUPPORTED' });
});
